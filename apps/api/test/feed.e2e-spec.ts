...
describe('Feed Module (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const [uA] = await db
      .insert(users)
      .values({
        email: 'usera_feed@srmist.edu.in',
        isVerified: true,
        status: 'ACTIVE',
        role: 'STUDENT',
      })
    ...
    const [uB] = await db
      .insert(users)
      .values({
        email: 'userb_feed@srmist.edu.in',
        isVerified: true,
        status: 'ACTIVE',
        role: 'STUDENT',
      })
    ...
    const [uC] = await db
      .insert(users)
      .values({
        email: 'userc_feed@srmist.edu.in',
        isVerified: true,
        status: 'ACTIVE',
        role: 'STUDENT',
      })
    ...
    const [uD] = await db
      .insert(users)
      .values({
        email: 'userd_feed@srmist.edu.in',
        isVerified: true,
        status: 'ACTIVE',
        role: 'STUDENT',
      })
  ...
  describe('Feed Candidate Generation & Personalization', () => {
    ...
    it('should seed posts for Student A, B, C, D', async () => {
      ...
      const resBConn = await request(app.getHttpServer())
        ...
        .send({
          content: 'Connections only post by Student B',
      ...
      const resCPub = await request(app.getHttpServer())
        ...
        .send({
          content: 'Public post by Student C (Followed by A)',
          visibility: 'PUBLIC',
        });
