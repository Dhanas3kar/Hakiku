...
export class MessageMediaService {
  ...
  async requestMediaUpload(userId: string, mimeType: string, fileSize: number) {
    ...
    if (isImage && fileSize > 10 * 1024 * 1024) throw new BadRequestException('Image exceeds 10MB limit');
    if (isVideo && fileSize > 50 * 1024 * 1024) throw new BadRequestException('Video exceeds 50MB limit');
    if (isFile && fileSize > 25 * 1024 * 1024) throw new BadRequestException('File exceeds 25MB limit');

    const extension = mimeType.split('/')[1] || 'bin';
    const uniqueId = crypto.randomUUID();
    const objectKey = `messages/${userId}/${uniqueId}.${extension}`;

    const uploadUrlResult = await this.storageProvider.getPresignedUploadUrl(objectKey, mimeType);
