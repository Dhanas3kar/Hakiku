...
import { conversations, conversationParticipants, users, profiles } from '../../db/schema';
...
export class ConversationService {
  ...
  async getOrCreateConversation(userId: string, targetUserId: string) {
    ...
    const { userAId, userBId } = this.accessService.getCanonicalParticipants(userId, targetUserId);
  ...
  async listConversations(userId: string, cursorAt?: string, limit = 20) {
    ...
    const items = hasNextPage ? participantRows.slice(0, limit) : participantRows;
