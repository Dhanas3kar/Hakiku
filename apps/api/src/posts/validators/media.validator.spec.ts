import { MediaValidator, IMAGE_MAX_SIZE, VIDEO_MAX_SIZE } from './media.validator';
...
describe('MediaValidator (Unit)', () => {
  ...
  it('should throw BadRequestException for unsupported MIME type', () => {
    ...
    expect(() => MediaValidator.validate(buffer, 'application/pdf')).toThrow(BadRequestException);
  });
  ...
  it('should throw BadRequestException for invalid magic byte signature', () => {
    ...
    expect(() => MediaValidator.validate(invalidBuffer, 'image/jpeg')).toThrow(BadRequestException);
  });

  it('should throw BadRequestException for oversized image buffer', () => {
    const jpegHeader = Buffer.from([0xff, 0xd8, 0xff, 0xe0]);
    const oversizedBuffer = Buffer.concat([jpegHeader, Buffer.alloc(IMAGE_MAX_SIZE + 100)]);
    expect(() => MediaValidator.validate(oversizedBuffer, 'image/jpeg')).toThrow(BadRequestException);
  });
  ...
  it('should throw BadRequestException for oversized video buffer', () => {
    ...
    const oversizedBuffer = Buffer.concat([mp4Header, Buffer.alloc(VIDEO_MAX_SIZE + 100)]);
    expect(() => MediaValidator.validate(oversizedBuffer, 'video/mp4')).toThrow(BadRequestException);
  });
});
