...
export class FeedService {
  ...
  private async verifyActiveViewer(viewerId: string) {
    const [viewer] = await this.db.select().from(users).where(eq(users.id, viewerId)).limit(1);
    ...
    if (viewer.status !== 'ACTIVE') {
      throw new ForbiddenException(`Account is ${(viewer.status || '').toLowerCase()} and cannot retrieve feed`);
    }
  ...
  async getPersonalizedFeed(viewerId: string, query: FeedQueryDto) {
    ...
    const decodedCursor = query.cursor ? this.cursorService.decode(query.cursor) : undefined;

    // 1. Candidate Generation
    const candidatePostIds = await this.candidateService.getPersonalizedCandidates(viewerId);
    if (candidatePostIds.length === 0) {
      return { data: [], pagination: { nextCursor: null, hasMore: false } };
    }

    // 2. Batch Hydration (No N+1)
    const hydratedItems = await this.queryService.hydrateCandidatePosts(viewerId, candidatePostIds);

    // 3. Visibility & Block Filtering
    const visiblePostItems = await this.visibilityService.filterVisiblePosts(
      viewerId,
      hydratedItems.map((item) => ({ ...item.post, authorId: item.author.userId }))
    );
    const visiblePostIds = new Set(visiblePostItems.map((p) => p.id));
    const eligibleHydrated = hydratedItems.filter((item) => visiblePostIds.has(item.post.id));

    // 4. Ranking & Cursor Filtering
    const viewerContext = await this.queryService.getViewerContext(viewerId);
    const rankedItems = this.rankingService.rankFeedItems(eligibleHydrated, viewerContext, decodedCursor);
    ...
    if (hasMore && pageItems.length > 0) {
      ...
      nextCursor = this.cursorService.encode(lastItem.score, lastItem.post.createdAt, lastItem.post.id);
    }
  ...
  async getDiscoveryFeed(viewerId: string, query: DiscoverQueryDto) {
    ...
    const candidatePostIds = await this.candidateService.getDiscoveryCandidates(viewerId, query);
    if (candidatePostIds.length === 0) {
      return { data: [], pagination: { nextCursor: null, hasMore: false } };
    }
