...
export class UsernameService {
  ...
  validateUsername(username: string): string {
    ...
    if (!normalized || normalized.length < 3 || normalized.length > 30) {
      throw new BadRequestException('Username must be between 3 and 30 characters in length');
    }

    if (RESERVED_USERNAMES.has(normalized)) {
      throw new BadRequestException(`Username '${normalized}' is reserved and cannot be used`);
    }
    ...
    if (/\.\.|\_\_|\.\_|\_\./.test(normalized)) {
      throw new BadRequestException('Username cannot contain consecutive dots or underscores');
    }
