import { pgTable, uuid, varchar, boolean, timestamp, pgEnum, text, jsonb, primaryKey, check, index, uniqueIndex, integer } from 'drizzle-orm/pg-core';
import { sql } from 'drizzle-orm';

export const roleEnum = pgEnum('role', ['STUDENT', 'MODERATOR', 'ADMIN']);

export const statusEnum = pgEnum('status', ['ACTIVE', 'SUSPENDED', 'BANNED', 'DEACTIVATED']);
...
export const authSessionStatus = pgEnum('session_status', ['ACTIVE', 'ROTATED', 'REVOKED']);
...
export const eventEnum = pgEnum('event', ['OTP_SENT', 'OTP_VERIFIED', 'OTP_FAILED', 'LOGIN_SUCCESS', 'LOGIN_FAILED', 'LOGOUT', 'TOKEN_REFRESHED']);
...
export const connectionRequests = pgTable(
  ...
  (table) => ({
    ...
    receiverStatusIdx: index('idx_connection_requests_receiver_status').on(table.receiverId, table.status),
    senderStatusIdx: index('idx_connection_requests_sender_status').on(table.senderId, table.status),
    noSelfConnectionRequest: check('no_self_connection_request', sql`${table.senderId} <> ${table.receiverId}`),
  })
);
...
export const connections = pgTable(
  ...
  (table) => ({
    ...
    canonicalOrder: check('canonical_user_order', sql`${table.userAId} < ${table.userBId}`),
  })
);
...
export const profileVisibilityEnum = pgEnum('profile_visibility', ['PUBLIC', 'CONNECTIONS_ONLY', 'PRIVATE']);
...
export const profiles = pgTable(
  ...
  (table) => ({
    usernameLowerIdx: uniqueIndex('idx_profiles_username_lower').on(sql`lower(${table.username})`),
    campusDeptIdx: index('idx_profiles_campus_dept').on(table.campus, table.department),
    batchGradIdx: index('idx_profiles_batch_grad').on(table.batchYear, table.graduationYear),
    validUsernameCheck: check('valid_username_format', sql`${table.username} ~ '^[a-z0-9](?:[a-z0-9._]*[a-z0-9])?$'`),
  })
);
...
export const skills = pgTable(
  ...
  (table) => ({
    nameLowerIdx: uniqueIndex('idx_skills_name_lower').on(sql`lower(${table.name})`),
  })
);
...
export const interests = pgTable(
  ...
  (table) => ({
    nameLowerIdx: uniqueIndex('idx_interests_name_lower').on(sql`lower(${table.name})`),
  })
);
...
export const postVisibilityEnum = pgEnum('post_visibility', ['PUBLIC', 'CONNECTIONS_ONLY', 'PRIVATE']);
...
export const posts = pgTable(
  ...
  (table) => ({
    authorCreatedIdx: index('idx_posts_author_created').on(table.authorId, table.createdAt),
    createdAtIdx: index('idx_posts_created_at').on(table.createdAt),
    activePostsIdx: index('idx_posts_active').on(table.authorId, table.deletedAt),
    likesCountCheck: check('check_likes_count_positive', sql`${table.likesCount} >= 0`),
    commentsCountCheck: check('check_comments_count_positive', sql`${table.commentsCount} >= 0`),
  })
);
...
export const postMedia = pgTable(
  ...
  (table) => ({
    postOrderIdx: index('idx_post_media_order').on(table.postId, table.displayOrder),
    fileSizeCheck: check('check_file_size_positive', sql`${table.fileSize} > 0`),
    displayOrderCheck: check('check_display_order_positive', sql`${table.displayOrder} >= 0`),
  })
);
...
export const pendingMediaUploads = pgTable(
  ...
  (table) => ({
    userAttachedIdx: index('idx_pending_media_user_attached').on(table.userId, table.isAttached),
  })
);
...
export const notifications = pgTable(
  ...
  (table) => ({
    recipientInboxIdx: index('idx_notifications_recipient_inbox').on(table.recipientId, table.createdAt),
    recipientUnreadIdx: index('idx_notifications_recipient_unread').on(table.recipientId, table.isRead),
  })
);
...
export const conversations = pgTable(
  ...
  (table) => ({
    canonicalOrder: check('canonical_conversation_order', sql`${table.userAId} < ${table.userBId}`),
    uniqueParticipants: uniqueIndex('idx_conversations_unique_participants').on(table.userAId, table.userBId),
  })
);
...
export const conversationParticipants = pgTable(
  ...
  (table) => ({
    ...
    userUpdatedIdx: index('idx_conversation_participants_user_updated').on(table.userId, table.lastReadAt), // Helps unread counts
  })
);
...
export const messages = pgTable(
  ...
  (table) => ({
    convoCreatedIdIdx: index('idx_messages_conversation_created_id').on(table.conversationId, table.createdAt, table.id),
...
export const messageMedia = pgTable(
  ...
  (table) => ({
    messageOrderIdx: index('idx_message_media_order').on(table.messageId, table.displayOrder),
    fileSizeCheck: check('check_message_file_size_positive', sql`${table.fileSize} > 0`),
    displayOrderCheck: check('check_message_display_order_positive', sql`${table.displayOrder} >= 0`),
  })
);
...
export const confessionStatusEnum = pgEnum('confession_status', ['DRAFT', 'PENDING_MODERATION', 'PUBLISHED', 'REJECTED', 'REMOVED', 'EXPIRED']);
export const pollStatusEnum = pgEnum('poll_status', ['PUBLISHED', 'CLOSED', 'REMOVED']);
export const reportStatusEnum = pgEnum('report_status', ['PENDING', 'REVIEWED', 'RESOLVED', 'DISMISSED']);
export const reportTargetTypeEnum = pgEnum('report_target_type', ['CONFESSION', 'POLL', 'POST', 'COMMENT', 'USER']);
...
export const confessions = pgTable(
  'confessions',
  {
    ...
    status: confessionStatusEnum('status').default('PENDING_MODERATION').notNull(),
  ...
  (table) => ({
    statusPublishedIdx: index('idx_confessions_status_published').on(table.status, table.publishedAt),
    campusPublishedIdx: index('idx_confessions_campus_published').on(table.campus, table.publishedAt),
  })
);
...
export const polls = pgTable(
  ...
  (table) => ({
    statusCreatedIdx: index('idx_polls_status_created').on(table.status, table.createdAt),
...
export const pollOptions = pgTable(
  ...
  (table) => ({
    voteCountCheck: check('check_poll_vote_count_positive', sql`${table.voteCount} >= 0`),
  })
);
...
export const communityReports = pgTable(
  ...
  (table) => ({
    targetTypeStatusCreatedIdx: index('idx_reports_type_status_created').on(table.targetType, table.status, table.createdAt),
    noSelfReportConfession: check('no_self_report_confession', sql`${table.targetType} != 'USER' OR ${table.reporterId} != ${table.targetId}`),
  })
);
