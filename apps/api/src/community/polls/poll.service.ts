...
export class PollService {
  ...
  async createPoll(userId: string, question: string, options: string[], isMultipleChoice: boolean = false, campus?: string) {
    if (options.length < 2 || options.length > 5) {
      throw new HttpException('Polls must have between 2 and 5 options', HttpStatus.BAD_REQUEST);
    }
  ...
  async vote(userId: string, pollId: string, optionId: string) {
    ...
    if (poll.status !== 'PUBLISHED' || (poll.endsAt && poll.endsAt < new Date())) {
      throw new HttpException('Poll is not active', HttpStatus.BAD_REQUEST);
    }

    return db.transaction(async (tx) => {
      // Check for existing votes
      const existingVotes = await tx.select().from(pollVotes).where(
        and(
      ...
      if (!poll.isMultipleChoice && existingVotes.length > 0) {
        throw new HttpException('You have already voted in this poll. Remove your vote to change it.', HttpStatus.CONFLICT);
      }

      if (poll.isMultipleChoice && existingVotes.some(v => v.optionId === optionId)) {
        throw new HttpException('You have already voted for this option.', HttpStatus.CONFLICT);
      }
