...
export class ConfessionQueryService {
  ...
  async getHeroConfession(viewerId: string) {
    ...
    return this.mapToPublic(safeHero);
  }

  async listConfessions(viewerId: string, limit: number = 20, offset: number = 0) {
    const activeBlocks = await this.getBlockedUserIds(viewerId);
    ...
    return paginated.map(c => this.mapToPublic(c));
  }
  ...
  private mapToPublic(confession: any) {
    return {
      id: confession.id,
