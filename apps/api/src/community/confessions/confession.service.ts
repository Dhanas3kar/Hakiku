...
export class ConfessionService {
  ...
  async submitConfession(userId: string, content: string, campus?: string) {
    ...
    if (hasSubmittedRecently) {
      throw new HttpException('You can only submit one confession every 12 hours.', HttpStatus.TOO_MANY_REQUESTS);
    }
  ...
  async deleteOwnConfession(userId: string, confessionId: string) {
    ...
    if (!confession) {
      throw new HttpException('Confession not found or not yours', HttpStatus.NOT_FOUND);
    }
