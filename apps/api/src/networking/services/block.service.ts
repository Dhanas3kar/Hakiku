...
import { blocks, follows, connections, connectionRequests } from '../../db/schema';
...
export class BlockService {
      ...
      await tx
        ...
        .where(
          and(
            ...
            or(
              and(eq(connectionRequests.senderId, blockerId), eq(connectionRequests.receiverId, blockedId)),
              and(eq(connectionRequests.senderId, blockedId), eq(connectionRequests.receiverId, blockerId))
            )
          )
        );
    });
  ...
  async isBlockedByTarget(currentUserId: string, targetUserId: string): Promise<boolean> {
    const result = await this.db
      .select()
      .from(blocks)
      .where(and(eq(blocks.blockerId, targetUserId), eq(blocks.blockedId, currentUserId)))
      .limit(1);

    return result.length > 0;
  }

  async isBlockedByMe(currentUserId: string, targetUserId: string): Promise<boolean> {
    const result = await this.db
      .select()
      .from(blocks)
      .where(and(eq(blocks.blockerId, currentUserId), eq(blocks.blockedId, targetUserId)))
      .limit(1);
