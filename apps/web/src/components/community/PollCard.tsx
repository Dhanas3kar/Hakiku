...
export function PollCard({ poll, hideQuestion = false }: { poll: Poll; hideQuestion?: boolean }) {
  ...
  const isPollActive = poll.isActive ?? true
  ...
  const voteMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      if (data?.options && data?.userVotedOptionIds) {
        queryClient.setQueryData(['polls'], (old: any) => {
          if (!old || !old.pages) return old
          return {
            ...old,
            ...
            return (
              <div key={option.id} className="relative">
                <button
                  ...
                  className={`w-full relative overflow-hidden flex items-center justify-between p-3 rounded-xl border text-left transition-all ${isVoted
                ...
                >
                  ...
                  {hasVoted && (
                    <div
                      className={`absolute inset-y-0 left-0 transition-all duration-500 ease-out rounded-xl ${isVoted ? 'bg-primary/25 border-r border-primary/40' : 'bg-surface-muted/90'
                        }`}
                  ...
                  <div className="flex items-center gap-3 relative z-10 min-w-0 flex-1">
                    <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border transition-colors ${isVoted ? 'border-primary bg-primary text-primary-foreground' : 'border-foreground-muted/40'
                      }`}>
