  ...
  return (
    <div className="flex min-h-[100dvh] w-full max-w-[100vw] overflow-x-hidden bg-background text-foreground">
      ...
      <div className="flex min-w-0 flex-1 flex-col max-w-full w-full">
        ...
        <main className="flex-1 pb-[calc(3.75rem+env(safe-area-inset-bottom,0px))] md:pb-0">
          <div
            className={cn(
              'mx-auto flex w-full min-w-0 gap-0',
              fullBleed
                ? 'max-w-none px-0 py-0 md:pl-0'
                : 'max-w-7xl md:justify-center md:gap-6 lg:gap-8 px-0 sm:px-6 lg:px-8 py-0 sm:py-6 lg:py-8',
            )}
          >
            <div
              className={cn(
                'min-w-0 w-full flex-1',
                fullBleed ? 'max-w-none' : 'max-w-full md:max-w-2xl lg:max-w-3xl',
              )}
            >
              {children}
            </div>
